 ...
 * checklist steps for one piece of work, in both grades. */
const store={};
...
console.log("=== both grades, every week: one step for the assignment and the writing ===");
...
{ const p=M.dayPlan("y2",4,"Thu");
  ...
  if(CL.dayMinutes(p.steps)!==32) fail.push("the lesson is now "+CL.dayMinutes(p.steps)+" minutes, not 32");
}
...
console.log("\n=== 3rd grade reads and writes in one step too ===");
{ Object.keys(store).forEach(k=>delete store[k]);
  const c=new C(); c.state.landed=true; c.state.year="y1"; c.state.week=4; c.state.day="Thu"; c.startDay();
  const v=c.renderVals();
  if(!(v.daySteps||[]).some(x=>/Read today's assignment and write it/.test(x.label))) fail.push("3rd grade has no merged writing row");
  if((v.daySteps||[]).some(x=>/^\d+\. Read today's assignment$/.test(x.label))) fail.push("3rd grade still has a separate assignment row");
  console.log("  3rd grade: "+M.dayPlan("y1",4,"Thu").steps.map(s=>s.label).join(" > "));
}

console.log(fail.length?("\nFAILURES:\n  "+fail.slice(0,8).join("\n  ")):"\nRESULT: both grades read and write the assignment as one step.");
