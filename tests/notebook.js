...
console.log("=== every lesson is named and says what to look for ===");
["y1","y2"].forEach(g=>{
  CL.ORDER.forEach(d=>{
...
{ const src=fs.readFileSync(__dirname+'/../word-voyagers.dc.html','utf8');
  ...
  const order=["lessonName","lessonLook","rdText","lessonDateLine","lessonTasks"];
  let last=-1;
  order.forEach(k=>{
  ...
  console.log("  LESSON -> look for -> passage -> date line -> numbered tasks");
}
